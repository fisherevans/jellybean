...
- Milestones + status: `gh api repos/fisherevans/jellybean/milestones | jq '.[] | {number, title, state, open: .open_issues, closed: .closed_issues}'`
- Open issues in a milestone:
  `gh issue list --repo fisherevans/jellybean --milestone "<exact title>" --state open`
- Single issue (body is load-bearing): `gh issue view <number> --repo fisherevans/jellybean`
- Full milestone scope + rationale (the rich context lives here, not in this file):
  `gh api repos/fisherevans/jellybean/milestones/<N> --jq .description`

There is deliberately no hardcoded "current focus" in this file - it
drifts. Discover what's in flight and what's next by querying the open
milestones above and cross-referencing the order in
[`docs/roadmap.md`](docs/roadmap.md). One caveat: **code can run ahead
of issue state.** Backend + admin plumbing for a milestone often lands
while that milestone's kid-side issues are still open, so confirm a
feature's real state against the code (grep `internal/curation`,
`internal/server`, `web/*/src`), not just the issue's open/closed flag.
...
Per-milestone scope, rationale, open questions, and status all live in
GitHub - in the milestone descriptions and issue bodies, not here. They
were duplicated into this file once and drifted badly; don't reintroduce
that. Query them with the commands under "Where work is tracked" above.
The descriptions are intentionally rich - read
`gh api repos/fisherevans/jellybean/milestones/<N> --jq .description`
before scoping or picking up work in a milestone.

- **Order** (what comes next, and why each slot): [`docs/roadmap.md`](docs/roadmap.md).
  The sequence isn't strictly numeric - M-AT slots between M5 and M6 for
  hardware-stutter pain, M14 between M6 and M9 because it's small and
  unblocks LLM-assisted tagging.
- **Labels are stable.** Milestone numbers (M1, M-AT, M10, ...) are sticky
  references used in commit messages, PR titles, and history. Don't
  renumber them even when the working order changes.
- **Far-future milestones** are scoped light on purpose; they get refined
  when their predecessor closes, informed by what we learned.
- **Capture discoveries where they'll be read.** When work in one
  milestone affects a future one's scope, append it to that milestone's
  GitHub description, not to this file:
  `gh api repos/fisherevans/jellybean/milestones/<N> -X PATCH -F description=@notes.txt`.

Historical note: the original M6 ("Optional Jellyfin tag mirror") was
scrapped - SQLite is already the source of truth and Jellyfin's tag
writes have known corruption issues. That M6 slot was reused for "Tags,
Favorites, and Profile Tag Filters."
...
```bash
# See all milestones + their open/closed counts (find what's in flight)
gh api repos/fisherevans/jellybean/milestones | jq '.[] | {number, title, state, open_issues, closed_issues}'

# Read a milestone's full scope/rationale before working in it
gh api repos/fisherevans/jellybean/milestones/<N> --jq .description

# List open work in a milestone (use the exact title from the command above)
gh issue list --repo fisherevans/jellybean --milestone "<exact title>" --state open

# View an issue (the body is load-bearing)
